...
func GetListEmployee(clientId string) ([]schema.Issue, error) {
	...
	query := /* sql */ `SELECT id, isseu_target_uri, isseu_image64, isseu_description, comment, isseu_status, created_at, updated_at, client_id, client_name  FROM isseus WHERE client_id = $1 ORDER BY created_at DESC`

	rows, err := db.Query(ctx, query, clientId)
